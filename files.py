...
def retrive_sims(sims, elements):
    """
    retrive similarity between elements in sims
    :param sims: dict, key-value: string-dict<string-value> ({entity1: {entity2: sim, }, }
    :param elements: a list of entities
    :return: sims of elements
    """
    ...
    for e1 in sims.keys():
        if e1 in elements:
            ...
            for e2 in sims[e1]:
                if e2 in elements:
                    ressim[e1][e2] = sims[e1][e2]
    resents = list(ressim.keys())
    for ent in resents:
        if len(ressim[ent]) == 0:
            del ressim[ent]
    return ressim
